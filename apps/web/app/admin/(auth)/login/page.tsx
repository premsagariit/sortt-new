  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        mustChange = !!(me?.must_change_password || me?.password_change_required);
      } catch (meError) {
        console.warn('Failed to fetch /api/users/me, falling back to dashboard', meError);
      }
      router.push(mustChange ? '/admin/create-password?mode=force' : '/admin/dashboard');
    } catch (err: unknown) {
      console.error(err);
          ...
          <div className="w-full md:w-1/2 bg-white p-10 md:p-16 flex flex-col justify-center min-h-screen overflow-y-auto">
            ...
            <form onSubmit={handleLogin} className="flex flex-col gap-5">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3.5 bg-navy text-white rounded-xl text-[14px] font-bold tracking-[0.02em] disabled:opacity-50 transition-opacity flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
