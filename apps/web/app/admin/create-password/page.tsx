 ...
 * ─────────────────────────────────────────────────────────────────
 * Admin Create Password Page.
 * Matches design from: sortt_admin_auth_ui.html (Screen 5)
 * ─────────────────────────────────────────────────────────────────
 */
...
function CreatePasswordContent() {
  ...
  const isForceChange = mode === 'force';

  useEffect(() => {
    if (!token && !isForceChange && !success) {
      setErrorInfo({ message: 'Invalid or missing access token.' });
    }
  }, [isForceChange, token, success]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isForceChange) {
        ...
        // Re-uses reset-password endpoint since the backend logic is identical
        ...
        if (!res.ok) {
          setErrorInfo({ message: data?.error || 'Failed to create password.' });
          return;
        }
      }
      ...
      setSuccess(true);
      setTimeout(() => {
        router.push(isForceChange ? '/admin' : '/admin/login');
      }, 3000);
    } catch (err: unknown) {
      setErrorInfo({ message: getErrorMessage(err) });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="min-h-screen bg-bg flex flex-col items-center py-12 px-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        <div className="w-full max-w-[460px] mx-auto mt-16">
          <div className="bg-tealLight border border-teal rounded-2xl p-10 text-center shadow-[0_2px_24px_rgba(0,0,0,0.07)]">
            ...
            <div className="text-[18px] font-extrabold text-teal mb-2 tracking-[-0.02em]">Password Created</div>
            <div className="text-[13px] text-slate leading-[1.6] mb-8">
              Your admin acccount is ready. You can now log into the admin portal using your new credentials.
            </div>
            <button
    ...
    <div className="min-h-screen bg-bg flex flex-col items-center py-12 px-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <div className="w-full max-w-[460px] mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-[24px] font-extrabold text-navy tracking-[-0.03em] mb-1">Create Password</h1>
          <p className="text-[13px] text-muted">{isForceChange ? 'Update your password to continue' : 'Welcome! Complete your admin setup'}</p>
        </div>
        ...
        <div className="bg-white border border-border rounded-2xl p-8 sm:p-10 shadow-[0_2px_24px_rgba(0,0,0,0.07)]">
          ...
          <div className="mb-6 px-4 py-3 rounded-lg bg-tealLight border border-teal/20 text-teal text-[12px] font-medium flex items-start gap-2">
            <span className="mt-[1px]">✓</span>
            <div>{isForceChange ? 'Your default password must be changed before you can access the admin portal.' : 'Access approved. Set up your password below to finalize your account setup.'}</div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <button
              type="submit"
              disabled={loading || (!token && !isForceChange)}
            ...
            >
              {loading ? 'Setting up...' : (isForceChange ? 'Update Password' : 'Create Password and Login')}
            </button>
          </form>
        </div>
