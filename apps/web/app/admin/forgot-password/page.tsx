 ...
 * Matches design from: sortt_admin_auth_ui.html (Screen 3)
 * ─────────────────────────────────────────────────────────────────
 */
...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email) {
      setErrorInfo({ message: 'Please enter your email.' });
      return;
    }
    setErrorInfo(null);
    setLoading(true);
    ...
    <div className="min-h-screen bg-bg flex flex-col items-center py-12 px-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <div className="w-full max-w-[460px] mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-[24px] font-extrabold text-navy tracking-[-0.03em] mb-1">Forgot Password</h1>
          <p className="text-[13px] text-muted">Admin account recovery</p>
        </div>
        ...
        <div className="bg-white border border-border rounded-2xl p-8 sm:p-10 shadow-[0_2px_24px_rgba(0,0,0,0.07)]">
          {!success ? (
            <>
                ...
                Enter the email address associated with your admin account. We&apos;ll send you a secure link to reset your password.
              </div>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-bg border border-border rounded-xl px-4 py-3 text-[14px] text-navy outline-none focus:border-navy transition-colors"
                  />
                </div>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-3.5 bg-navy text-white rounded-xl text-[14px] font-bold tracking-[0.02em] disabled:opacity-50 hover:bg-navySoft transition-colors mt-2"
                >
            ...
            <div className="text-center py-4">
              ...
              <div className="text-[18px] font-extrabold text-navy mb-2 tracking-[-0.02em]">Check your email</div>
