...
/**
 * app/admin/(portal)/active-aggregators/page.tsx
 * Professional active aggregators view with performance scoring, star ratings, and detail cards.
 */
...
function PerformanceBadge({ rating }: { rating: number }) {
  if (rating >= 4.5) return <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-teal text-white">⭐ Excellent</span>;
  if (rating >= 4.0) return <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-teal/20 text-teal border border-teal/30">✨ Very Good</span>;
  return <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-600 border border-emerald-200">👍 Good</span>;
}

export default function ActiveAggregatorsPage() {
  ...
  const excellent = aggregators.filter((a) => a.avg_rating >= 4.5).length;
  const veryGood = aggregators.filter((a) => a.avg_rating >= 4.0 && a.avg_rating < 4.5).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-navy tracking-tight">Active Aggregators</h1>
          <p className="text-[13px] text-muted mt-1">
            {loading
              ? 'Loading…'
              : `${aggregators.length} aggregator${aggregators.length !== 1 ? 's' : ''} with avg rating 3.5+ after 10+ orders`}
          </p>
        </div>
        ...
        {!loading && aggregators.length > 0 && (
          <div className="flex flex-wrap gap-2">
            ...
            {veryGood > 0 && (
              <span className="px-3 py-1.5 bg-emerald-100 text-emerald-600 text-[12px] font-bold rounded-xl border border-emerald-200">
                ✨ {veryGood} Very Good
              </span>
            )}
          </div>
      ...
      {!loading && aggregators.length === 0 && !error && (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h3 className="text-[16px] font-bold text-navy mb-1">No high-performing aggregators yet</h3>
          <p className="text-[13px] text-muted">Aggregators with 3.5+ average rating after 10+ orders will appear here.</p>
        </div>
      )}
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 flex-wrap">
                      ...
                      <PerformanceBadge rating={agg.avg_rating} />
                    </div>
                  ...
                  <div className="h-2 bg-bg border border-border rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all ${
                        agg.avg_rating >= 4.5 ? 'bg-teal' : agg.avg_rating >= 4.0 ? 'bg-emerald-500' : 'bg-emerald-400'
                      }`}
